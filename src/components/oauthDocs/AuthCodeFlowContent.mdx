...
```plaintext
https://sandbox-api.va.gov{{props.propObj.apiDef?.oAuthInfo?.baseAuthPath ?? '/oauth2/{api}/v1'}}/authorization?
...
```http
POST {{props.propObj.apiDef?.oAuthInfo?.baseAuthPath ?? '/oauth2/{api}/v1'}}/token HTTP/1.1
...
```http
POST {{props.propObj.apiDef?.oAuthInfo?.baseAuthPath ?? '/oauth2/{api}/v1'}}/manage HTTP/1.1
Host: sandbox-api.va.gov
```
...
```http
POST {{props.propObj.apiDef?.oAuthInfo?.baseAuthPath ?? '/oauth2/{api}/v1'}}/revoke HTTP/1.1
Host: sandbox-api.va.gov
Content-Type: application/x-www-form-urlencoded
Authorization: Basic { base64 encoded *client_id* + ':' + *client_secret* }
...
```http
DELETE {{props.propObj.apiDef?.oAuthInfo?.baseAuthPath ?? '/oauth2/{api}/v1'}}/grants HTTP/1.1
Host: sandbox-api.va.gov
Content-Type: application/x-www-form-urlencoded
